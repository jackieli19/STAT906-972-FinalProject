function P=HestonPut(St,K,r,sig,T,vt,kap,th,lda,rho)
%--------------------------------------------------------------------------
...
maxphi=50;
phi=(eps:dphi:maxphi)';
phi1=repmat(phi,1,length(St));

f1 = CF_SVj(log(St),vt,T,r,kap*th,0.5,kap+lda-rho*sig,rho,sig,phi);
P1 = 0.5+(1/pi)*sum(real(exp(-1i*phi1*log(K)).*f1./(1i*phi1))*dphi);
f2 = CF_SVj(log(St),vt,T,r,kap*th,-0.5,kap+lda,rho,sig,phi);
P2 = 0.5+(1/pi)*sum(real(exp(-1i*phi1*log(K)).*f2./(1i*phi1))*dphi);
